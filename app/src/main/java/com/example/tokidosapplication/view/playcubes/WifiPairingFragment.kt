        ...
        override fun onReceive(context: Context, intent: Intent) {
            when (intent.action) {
                ...
                BluetoothLeService.ACTION_DATA_AVAILABLE->{
                    ...
                    val callbackData = intent.getStringExtra(BluetoothLeService.EXTRA_DATA)
                    //val callbackDataPassword = intent.getStringExtra(BluetoothLeService.EXTRA_DATA_SSID)
                    println("ACTION_DATA_AVAILABLE: $callbackData $inputWifiPassword $inputWifiSSID")
                    if(callbackData?.contains("Not Connected") == true){
                        updateUIFail()
                        return
...
//        activity?.unregisterReceiver(gattUpdateReceiver)
    ...
    private fun sendCredentials(wifi: Wifi?, password: String){
        ...
        jobSendWifiCredentials = CoroutineScope(Dispatchers.IO).launch {
            val characteristicsMap = instance.getBluetoothLeService()?.getCharacteristicsMap()
            println("sda: ${characteristicsMap}")
            if (characteristicsMap != null) {
                //write wifi credentials(ssid & password) to connected BLE device
                characteristicsMap[wifiSSID]?.let { instance.getBluetoothLeService()?.writeCharacteristic(it,inputWifiSSID!!) }
                delay(2000)
                ...
                characteristicsMap[wifiConnection]?.let {
                    val result = instance.getBluetoothLeService()?.readCharacteristic(it)
                    if (result == false) {
                        withContext(Dispatchers.Main){
                            updateUIFail()
                        }
